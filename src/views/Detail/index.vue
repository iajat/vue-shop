        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="attrbute in spuSaleAttrList" :key="attrbute.id">
                <dt class="title">{{attrbute.saleAttrName}}</dt>
                <dd changepirce="0" @click="isChencked(attrbuteValue,attrbute.spuSaleAttrValueList)" :class="{active:attrbuteValue.isChecked==1}" v-for="attrbuteValue in attrbute.spuSaleAttrValueList" :key="attrbuteValue.id">{{attrbuteValue.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input v-model.number="number" @change="isNumber"  autocomplete="off"  class="itxt" />
                <a href="javascript:" class="plus" @click="number++">+</a>
                <a href="javascript:" class="mins" @click="number>=2?number--:number">-</a>
              </div>
              <div class="add">
...
export default {
  ...
  methods:{
    isNumber(){
      let isn = /^[0-9]*$/
      if(!isn.test(this.number)){
        alert('你输入的什么JB东西')
        this.number=1
      }
    },
    isChencked(attrbuteValue,spuSaleAttrValueList){
      spuSaleAttrValueList.forEach(element => {
        element.isChecked=0
      });
      attrbuteValue.isChecked=1
    },
    AddCart(){
      this.$store.dispatch("AddOrUpdataCart",{skuId:this.$route.params.skuid, skuNum:this.number})
      .then(
        (resolve)=>{alert  (resolve);
        sessionStorage.setItem('SKUID',JSON.stringify(this.skuInfo) )
        this.$router.push({
          name:'AddCartSuccess',
